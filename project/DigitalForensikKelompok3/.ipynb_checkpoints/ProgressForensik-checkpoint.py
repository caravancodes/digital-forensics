...
window.title("sofware digital forensik")
...
def model1():
    deviceModel = os.popen('adb shell getprop ro.product.model').read()
    label2 = tkinter.Label(window, bg="#FBE9C1", text=(deviceModel))
...
def backup():
    ...
    MsgBox = tk.messagebox.askquestion('Backup Data', 'Are you sure you want to Backup Data',
                                       icon='warning')
    if MsgBox == 'yes':
         os.system("adb backup -apk -shared -all -nonsystem -f \"" + folder1 + "\\backup.ab\"")
    else:
        tk.messagebox.showinfo('Return', 'You will now return to the application screen')
...
def backup_device():
    ...
    MsgBox = tk.messagebox.askquestion('Backup Data', 'Are you sure you want to Backup Data',
                                       icon='warning')
    if MsgBox == 'yes':
        print("adb backup -all -f C:\Backups\\backup.ab")
...
def backup_storage():
    MsgBox = tk.messagebox.askquestion('Backup Data', 'Are you sure you want to Backup Data',
                                       icon='warning')
    if MsgBox == 'yes':
        os.system("adb backup -noapk -shared -nosystem -f C:\Backups\\backup.ab")
        os.system("ab-decrypt C:\Backups\\backup.ab C:\Backups\\backup.tar")
    else:
        tk.messagebox.showinfo('Return', 'You will now return to the application screen')
...
def backup_system():
    MsgBox = tk.messagebox.askquestion('Backup Data', 'Are you sure you want to Backup Data',
                                       icon='warning')
    if MsgBox == 'yes':
        os.system("adb backup -apk -shared -all -system -f C:\Backups\\backup.ab")
        os.system("ab-decrypt C:\Backups\\backup.ab C:\Backups\\backup.tar")
    else:
        tk.messagebox.showinfo('Return', 'You will now return to the application screen')
